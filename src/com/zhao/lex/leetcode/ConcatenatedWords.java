...
//            Map<String, List<String>> map = new HashMap<String, List<String>>();
            List<String> result = dfs(word, words, map);
    ...
    private static List<String> dfs(String s, String[] wordList, Map<String, List<String>> map) {
        ...
        for(String word : wordList) {
            if (word != "" && s.startsWith(word)) {
                List<String> subList = dfs(s.substring(word.length()), wordList, map);
...
//    */
    ...
    public static List<String> findAllConcatenatedWordsInADict(String[] words) {
        ...
        for (String word:words) {
            if (split(word,0)>1) ans.add(word);
        }
        return ans;
    }

    private static int split(String word,int index){
        TrieNode node = root;
        for (int i=index;i<word.length();i++){
            if (node.next[word.charAt(i)-'a']==null) return 0;
            node = node.next[word.charAt(i)-'a'];
            if (node.s.length()!=0){
                int n = split(word,i+1);
                if (n>0) return 1+n;
            }
        }
        return node.s.length()==0?0:1;
    }
...
//        String[] words = new String[]{"apple", "ap", "pl"};
